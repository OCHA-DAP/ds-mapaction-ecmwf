...
client = cdsapi.Client(
    url=os.getenv("ECMWF_API_URL"), key=os.getenv("ECMWF_API_KEY")
)
...
single_request = create_single_request(
    available_years, range(1, 13), all_leadtimes
)
...
def download_single_request(request, path):
    ...
    client.retrieve(
        "seasonal-monthly-single-levels", request, request["target"]
    )
