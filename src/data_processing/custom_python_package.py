...
def _load_climate_data(input_file_path, bbox, filter_value = None):
	...
	filter_value: int, When loading ECMWF data, value used to filter ensemble model number 
...
def _create_reference_grid(input_df, admin_df, admin_code_label):
	...
	admin_df.rename(columns = {admin_code_label: 'adm_pcode'}, errors = 'ignore', inplace = True)
	...
	grid_df = gpd.GeoDataFrame(grid_df, geometry=gpd.points_from_xy(grid_df.longitude, grid_df.latitude), crs="EPSG:4326")
	grid_df = grid_df[['pixel_geom_id','adm_pcode','geometry']]
...
def _compute_era5_climatology(input_df, geom_id_label):
	...
	geom_id_label: str, Name of the column containing the unique geometry id. It can be either pixel_geom_id or adm_pcode
	...
	is below the climatology X% precipitation quantile. The name "prob" is used to keep the same nomenclature of the ECMWF dataset where 
...
def pre_process_ecmwf_data(input_file_path, admin_boundary_file_path, ref_grid_file_path, pixel_output_file_path, adm_output_file_path, admin_code_label):
	...
	for batch_number in range(0,51):
		...
		#Link_df is a MxN link table between grid points and administrative boundaries. 
		...
		batch_data_adm_df = link_df.groupby(['adm_pcode','number','valid_time_year','valid_time_month','lead_time'])['tp_mm_day'].mean().reset_index()
...
def pre_process_era5_data(era5_file_path, ecmwf_file_path, admin_boundary_file_path, ref_grid_file_path, pixel_output_file_path, adm_output_file_path):
	...
	data_adm_df = geo_df.groupby(['adm_pcode','valid_time_year','valid_time_month','lead_time'])['tp_mm_day'].mean().reset_index()
