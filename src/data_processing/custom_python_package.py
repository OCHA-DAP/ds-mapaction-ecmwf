# TODO: check if cfgrib unused import can be removed
...
def _load_climate_data(
    input_file_path: str, batch_load: bool, batch_number: int
):
    """
        Loads climate data from and input file and returns it on a dataframe
        format. For the moment only grib files are accepted but this function
        can be adapted to cover other file formats.


    Parameters
    ----------
        input_file_path: str, Path to the climate data to be loaded
        batch_load: bool, Indicates if a batch processing should be done.
        Used to load heavy ECMWF datasets
        batch_number: int, In the case of batch processing, indicates which
                            model is to be load for a ECMWF dataset
    ...
    # Load grib data into pandas dataframe
    # TODO: Change this function if another data format is used
    input_xr = xr.open_dataset(input_file_path, engine="cfgrib")

    # If batch_load = True, loads only one model from a ECMWF dataset
    if batch_load:
        df = (
            input_xr.where(input_xr["number"] == batch_number, drop=True)
            .to_dataframe()
            .dropna()
            .reset_index()
        )
...
def _regrid_climate_data(era5_file_path: str, ecmwf_file_path: str):
    """
        Re-gridding of a climate dataset following a reference dataset.
...
def _create_reference_grid(
    ...
    admin_gdf.rename(
        columns={admin_code_label: "adm1_pcode"}, errors="ignore", inplace=True
    )

    admin_gdf = admin_gdf[["adm1_pcode", "geometry"]]
    # Finds all grid points that are close to the admin boundary
    # (intersection with the square buffer)
    grid_df = grid_df.sjoin(admin_gdf, how="inner", predicate="intersects")
...
def _compute_era5_climatology(
    input_df: pd.DataFrame, geom_id_label: str
) -> pd.DataFrame:
    """
    ...
    ----------
        ...
        geom_id_label: str, Name of the column containing the unique
                            geometry id. It can be either pixel_geom_id
                            or adm1_pcode

        Returns
    -------
        DataFrame, Adds two set of columns (climatology_q and prob_q)
                    to the input DataFrame. climatology_qX is the X%
                    precipitation quantile for a given location and month.
                    prob_qX has a value of 1 (0 otherwise) if,
                    for a given location, month and year, the precipitation
                    level is below the climatology X% precipitation quantile.
                    The name "prob" is used to keep the same nomenclature
                    of the ECMWF dataset where an ensemble model is used
    ...
    # Computes, for a given location, month and year, if the precipiation level
...
def pre_process_ecmwf_data(
    ...
    """
        ...
        administrative boundaries. Finally, computes a reference grid by
        linking grid points to administrative boundaries. The process is
        executed ensemble model per ensemble model (out of 51 in total) to
        limit memory use.
    ...
    ----------
        ...
        ref_grid_file_path: str, Path where the reference grid, combining
                                both grid points and admin boundaries link,
                                is to be exported
        pixel_output_file_path: str, Path where the processed file at the
                                grid point level should be exported
        adm_output_file_path: str, Path where the processed file at the
                                admin boundary level should be exported
        admin_code_label: str, Column name to be used as an unique admin code
    ...
    for batch_number in range(0, 51):
        ...
        df = _load_climate_data(
            input_file_path, batch_load=True, batch_number=batch_number
        )
        ...
        # boundaries. The groupby allows to drop grid points duplicate in
        # the first case or aggregate into admin boundaries in the second one
...
def pre_process_era5_data(
    ...
    """
        ...
        Also adapt columns names, precipitation units and link grid points
        to administrative boundaries. Finally, computes the average climatology
        (per location and month) and export the resulting DataFrame
        to a parquet file.


    Parameters
    ----------
        input_file_path: str, Path to the ERA5 climate data grib file to be
                                processed. This dataset should have been
                                previously regridded to the ECMWF grid
    ...
    # Load both ERA5 and ECMWF initial grib file, re-grid the ERA5 dataset
    # to match the one from ECMWF and returns it on a DataFrame format
    ...
    data_adm_df = (
        geo_df.groupby(
            ["adm1_pcode", "valid_time_year", "valid_time_month", "lead_time"]
        )["tp_mm_day"]
        .mean()
        .reset_index()
    )
...
def ecmwf_bias_correction(
    ...
    """
        Compute the bias between the average precipitation prediction (ECMWF)
        and ground truth (ERA5) per location, month, model and lead time.
    ...
    # Compute bias between the average precipitation per
    # location, month, model and lead time
    avg_df = pd.merge(
        ecmwf_avg_df,
        era5_avg_df,
...
def compute_quantile_probability(
    ...
    """
        Computes the probability of the ECMWF predicted precipitation being
        under different quantile thresholds foe every location, month and year.
        Quantiles thresholds are computd from ERA5 climatology for a given
        location and month. Different thresholds are used following the
        quantiles values (quantiles 50%, 33%, 25% and 20%).
        Probability is based on the share of ECMWF model predicting a below
        quantile value. Individual predictions bias and mean absolute error
        are also computed.
    ...
    ----------
        ...
        output_file_path: str, Path where the computed quantiles probability
                                and bias / maae score ECMWF file should
                                be exported
    ...
    # ERA5 climatology for the same location and month.
    ...
    # Combine all model predictions into a single average value plus
    # the probability of it being under each climatology quantile.
    ecmwf_prob_df = (
